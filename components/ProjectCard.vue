<template>
  ...
  >
    ...
    <h3 class="text-l truncate">{{ project.purpose }}</h3>
    <h4 class="text-md truncate pb-4">{{ project.description }}</h4>
    <a
      :href="`mailto:${project.contact}`"
      class="font-medium dark:text-rose-quartz text-classic-blue hover:underline"
      >{{ project.contact }}</a
    >
